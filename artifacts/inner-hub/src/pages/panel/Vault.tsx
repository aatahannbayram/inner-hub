...
const ACCESS_STYLE: Record<AccessLevel, { icon: React.ComponentType<{ className?: string }>; color: string; labelKey: string }> = {
  ...
  davetli: { icon: Globe, color: "text-[var(--success-ink)]", labelKey: "vault.inviteOnly" },
};
...
function docTypeLabel(type: DocType, t: (k: string) => string) {
  const map: Record<DocType, string> = {
    ...
    "Araştırma": t("vault.typeResearch"),
    "Not": t("vault.typeNote"),
    "Şablon": t("vault.typeTemplate"),
    "Kod": t("vault.typeCode"),
    "Rapor": t("vault.typeReport"),
  };
...
function DocCard({ doc }: { doc: VaultDoc }) {
  ...
  const onDownload = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setDlError(err.message ?? t("vault.downloadFailed"));
    } finally {
      setDlBusy(false);
    }
  };
    ...
    <article className="group relative flex h-full flex-col overflow-hidden panel-glass p-4 transition-colors hover:border-[var(--ink)]/28 sm:p-5">
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-[var(--ink)]/[0.06] pt-3 pl-1">
        <ul className="flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[10px] text-[var(--ink-muted)]">
          {doc.pages ? <li>{t("vault.pages", { n: doc.pages })}</li> : null}
          <li>{t("vault.views", { n: doc.views })}</li>
          <li className="inline-flex items-center gap-1">
            <Clock className="size-2.5" aria-hidden />
            {doc.updatedDays === 0 ? t("vault.today") : t("vault.daysAgo", { n: doc.updatedDays })}
          </li>
        </ul>
        {doc.hasFile && (
          <button
            ...
            disabled={dlBusy}
            className="inline-flex min-h-10 shrink-0 items-center gap-1.5 bg-[var(--ink)] px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest text-[var(--bone)] transition-opacity hover:opacity-85 disabled:opacity-40"
          >
...
function UploadPrompt({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json.error ?? t("vault.saveFailed"));
      ...
      if (file && docId) {
        ...
        if (!up.ok) throw new Error(upJson.error ?? t("vault.uploadFailed"));
      }
      ...
      setError(e.message ?? t("vault.saveFailed"));
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className="space-y-4 px-6 pb-8">
          ...
          <label className="flex cursor-pointer flex-col gap-1 border border-dashed border-[var(--ink)]/20 px-3 py-3 transition-colors hover:border-[var(--ink)]/40">
            <span className="font-mono text-label uppercase tracking-widest text-[var(--ink-muted)]">
              {t("vault.fileOptional")}
            </span>
            <span className="text-sm text-[var(--ink-body)]">
              {file ? `${file.name} · ${formatBytes(file.size)}` : t("vault.filePick")}
            </span>
            <input
              type="file"
              className="sr-only"
          ...
          <div className="flex flex-wrap gap-1.5">
            {DOC_TYPES.filter((ty): ty is DocType => ty !== "Tümü").map((ty) => (
              <button
                key={ty}
                type="button"
                ...
                className={[
                  ...
                  docType === ty ? "border-[var(--ink)] bg-[var(--ink)] text-[var(--bone)]" : "border-[var(--ink)]/10 text-[var(--ink-muted)]",
                ].join(" ")}
              >
                {docTypeLabel(ty, t)}
              </button>
          ...
          <div>
            <p className="mb-2 font-mono text-label uppercase tracking-widest text-[var(--ink-muted)]">{t("vault.accessLevel")}</p>
            <div className="flex gap-2">
              {(["özel", "topluluk", "davetli"] as AccessLevel[]).map((a) => {
                const cfg = ACCESS_STYLE[a];
                const Icon = cfg.icon;
                return (
                  <button
                    key={a}
                  ...
                  >
                    ...
                    <span className="font-mono text-label uppercase tracking-widest text-[var(--ink-body)]">{t(cfg.labelKey)}</span>
                  </button>
                );
              })}
            </div>
...
export default function Vault() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      {isLoading && docs.length === 0 && <LoadingBlock label={t("vault.loading")} />}
      {isError && (
        <ErrorState
          message={error instanceof Error ? error.message : t("vault.loadError")}
          onRetry={() => refetch()}
        />
      )}
      ...
      <FadeIn delay={0.03}>
        <div className="relative overflow-hidden panel-glass">
          ...
          />
          <div className="pointer-events-none absolute inset-0 flex flex-col justify-end p-4 sm:p-6 md:p-8">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        {[
          { label: t("vault.statTotal"), value: totalDocs },
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="-mx-1 flex gap-1.5 overflow-x-auto px-1 pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {DOC_TYPES.map((ty) => (
            ...
            >
              {ty === "Tümü" ? t("common.all") : docTypeLabel(ty, t)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 border-t border-[var(--ink)]/[0.08] pt-4">
        {(Object.entries(ACCESS_STYLE) as [AccessLevel, typeof ACCESS_STYLE[AccessLevel]][]).map(
          ([key, cfg]) => {
            const Icon = cfg.icon;
            return (
              <div key={key} className={`flex items-center gap-1.5 ${cfg.color}`}>
                <Icon className="size-3" />
                <span className="font-mono text-[9px] uppercase tracking-widest">{t(cfg.labelKey)}</span>
              </div>
            );
          },
        )}
        <p className="ml-auto hidden font-mono text-[10px] uppercase tracking-widest text-[var(--ink-subtle)] sm:block">
          <span lang="en">inner·vault</span> · {t("vault.footer")}
        </p>
      </div>
    </div>
  );
